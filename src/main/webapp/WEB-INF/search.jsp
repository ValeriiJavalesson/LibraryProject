...
<body>
	...
	<div>
	<c:set var="number_of_books" value="0"></c:set>
	<c:forEach var="self" items="${selfs}">
		<c:set var="number_of_books_on_self" value="${fn:length(self)}"></c:set>	
		<c:set var="number_of_books" value="${number_of_books + number_of_books_on_self}"></c:set>	
	</c:forEach>
		<c:if test="${searchedword ne ''}">
			<div class="searchedword bold h5 ms-4 text-light">Результати пошуку за запитом <b>"${searchedword}"</b>: ${number_of_books} шт</div>
		</c:if>
	</div>
			...
			<div class="genre_self">
				<div class="books_self" id="product-cards">
					<c:forEach var="book" items="${self}">
						<div class="book" data-pages="${book.numberOfPages}" 
								data-author="${book.author}" data-name="${book.name}"
